...
Inherits URLConnection
	...
	#tag Note, Name = About
		Class: GroqAPI
		API Key: You can get one from: https://console.groq.com/keys
		
		Purpose:
		The GroqAPI class is designed to facilitate interaction with the Groq API for AI model requests.
		It provides methods to send chat completion requests and retrieve a list of available models, handling the necessary HTTP communication under the hood.
		...
		CheckContextWindowUsage: Monitors the usage of the model's context window.
		
		Event Hooks:
		ResponseReceived: Allows developers to process and display data received from successful API responses.
		ErrorReceived: Provides a mechanism to handle and respond to errors encountered during API requests.
		The class also manages authentication via an API key and configures the necessary HTTP headers for secure communication with the API. It is designed to be flexible and extensible, making it suitable for various applications that require AI-driven interactions.
	#tag EndNote
	...
	#tag Note, Name = Example
		...
		// Detach the event handler after processing to avoid multiple calls
		RemoveHandler sender.ResponseReceived, AddressOf HandleResponseReceived
		
		If ResponseType = "chat.completion" Then
		  // Iterate through the messages in the response
		  For i As Integer = 0 To AvailableData.Count - 1
		    Var message As JSONItem = AvailableData.Child(i)
		    Var role As String = message.Value("role")
		    Var content As String = message.Value("content")
		
		    // Display the response in the console or UI
		    System.DebugLog(role + ": " + content)
		  Next
		ElseIf ResponseType = "available.models" Then
		  // Iterate through the list of available models
		  For i As Integer = 0 To AvailableData.Count - 1
		    Var model As JSONItem = AvailableData.Child(i)
		    Var modelName As String = model.Value("id")
		
		    // Display the model name in the console or UI
		    System.DebugLog("Model: " + modelName)
		  Next
		End If
		End Sub
		...
		// Detach the event handler after processing to avoid multiple calls
		RemoveHandler sender.ErrorReceived, AddressOf HandleErrorReceived
		
		System.DebugLog("Error: " + HTTPStatus.ToString + " - " + content)
		End Sub
